...
public class UserUI {
	...
	public void displayMatches(ArrayList<Integer> matchId, ArrayList<String> teamOne, ArrayList<String> teamTwo) {
		...
		for(index =0 ;index<matchId.size();index++) {
			stringBuilder.append(Integer.toString(matchId.get(index))+" "+teamOne.get(index)+" "+teamTwo.get(index)+"\n");
		}
	...
	public void displayResult(ResultBoard resultBoard) {
		...
		for(index = 0; index<userId.size();index++) {
			stringBuilder.append(Integer.toString(userId.get(index))+" "+names.get(index)+" "+Integer.toString(matchPoints.get(index))+"\n");
		}
		logger.info(stringBuilder.toString());
	}
	...
	public void displayPlayerNamesList(ArrayList<Player> playersList) {
		...
		for(index = 0;index < playersList.size();index++) {
			stringBuilder.append(playersList.get(index).getPlayerId()+" "+playersList.get(index).getplayerName() +" " +playersList.get(index).getplayerType()+" "+playersList.get(index).getplayerRating()+"\n");
		}
		logger.info(stringBuilder.toString());
	...
	public void displayMessages(ArrayList<Message> messages) {
		...
		for(index = 0;index < messages.size();index++) {
			stringBuilder.append(messages.get(index).getUserName()+" "+messages.get(index).getMessage()+" "+messages.get(index).getInsertedTime()+"\n");
		}
		logger.info(stringBuilder.toString());
	}
