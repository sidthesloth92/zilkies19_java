...
public class QueryConstants {
	...
	public static final String VALIDATE_USER = "select user_name, email_id, expense_limit,u_id from Users where user_name = ? and password = md5(?)";
	...
	public static final String SELECT_GROUP_EXPENSES = "select amountPaid, category_id, type_id ,description, Group_Expenses.created_at, Group_Expenses.updated_at from Expense_Members join Group_Expenses on Expense_Members.payment_id = Group_Expenses.payment_id  where Expense_Members.u_id = ? and pending = 0";
	...
	public static final String FIND_GROUP_BALANCES = "select u_id as id, sum(totalAmount),(select sum(amount) from Group_Expenses where u_id = id group by(u_id) ) from Expense_Members group by u_id";
