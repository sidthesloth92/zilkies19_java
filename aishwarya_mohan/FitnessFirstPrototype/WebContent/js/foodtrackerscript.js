	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			for (var i = 0; i < choiceList.length; i++) {
				...
				var nodeNameText = document.createTextNode(choiceList[i].foodName);
...
function updateMeal(){
	...
	var mealtimeinput = document.getElementsByClassName("meal-time")[0].selectedIndex;
	...
	xhttp.open("POST", "/FitnessFirstPrototype/FoodTrackerServlet", true);
	xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
	xhttp.send("foodslist=" + JSON.stringify(foodslist) +"&date="+dateinput+"&mealtime="+mealtimeinput);
}
