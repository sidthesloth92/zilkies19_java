...
import io.ztech.onlinebidding.dao.PresentBidItemIdFromBase;
...
public class BidAnItemDelegate {
	...
	PresentBidItemIdFromBase bidItemId = new PresentBidItemIdFromBase();
	int bidderId, customerIdOfBidder;

	public boolean checkApplicant(String bidItemId, String username) throws Exception {
		try {
	...
	public boolean checkAvailable(String bidItemId, String username) throws Exception {
		try {
			customerId = idRetreive.customerIdRetrieve(username);
			if (customerId != 0) {
				return true;
	...
	public boolean checkTimeAvailable(String bidItemId, String userName) throws Exception {
		try {
	...
	public void calculate(String bidItemId, String username) throws Exception {
		try {
			customerId = idRetreive.customerIdRetrieve(username);
			price = priceRetreive.priceRetrive(bidItemId);
			timeList = timeRetreive.retreiveTime(bidItemId);
	...
	public ArrayList<String> presentBidItemId(String itemId, String categoryId) throws Exception {
		try {
			return bidItemId.presentBidItemId(itemId, categoryId);
		} catch (Exception e) {
			throw e;
		}
	}
