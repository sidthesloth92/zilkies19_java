...
public class CreateSchedule extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			System.out.println("tournament list is:"+tournamentDetails);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Inside create Schedule doPOST");
		response.setHeader("Cache-Control", "no-cache, no-store, must-revalidate"); // HTTP 1.1.
		response.setHeader("Pragma", "no-cache"); // HTTP 1.0.
		response.setDateHeader("Expires", 0);
		...
		try {
			...
			System.out.println("schedule:"+arrayList);
			for (int i = 0; i < arrayList.size(); i++) {
				String array[] = arrayList.get(i).split("v");
				String teamid="";
				teamid = tournamentMatches.getTeamId(array);
				String arr[]=teamid.split(",");
				System.out.println("teamid1:"+Integer.parseInt(arr[0]));
				System.out.println("teamid2:"+Integer.parseInt(arr[1]));
				Schedule scheduleobj = new Schedule();
				scheduleobj.setTeamOneId(Integer.parseInt(arr[0]));
				scheduleobj.setTeamTwoId(Integer.parseInt(arr[1]));
