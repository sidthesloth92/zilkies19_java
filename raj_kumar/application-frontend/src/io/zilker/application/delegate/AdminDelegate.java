...
import io.zilker.application.beans.ControllerResponse;
...
public class AdminDelegate {
	...
	public void addNewProject(Project project) throws Exception {
		...
		try {
			...
			ControllerResponse controllerResponse = gson.fromJson(response.toString(), ControllerResponse.class);
			if (controllerResponse.getStatus() == 500) {
				throw new SystemException(DisplayConstants.SYSTEM_EXCEPTION);
			}
		} catch (SystemException e) {
			logger.info(DisplayConstants.SYSTEM_EXCEPTION);
		} catch (Exception e) {
	...
	public Contractor getContractor(int contractorID) {
		...
		try {
			...
			// Converting from JSON string to Bean Object
